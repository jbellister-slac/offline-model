...
COPY environment.yml /offline-model/environment.yml

RUN apt-get update && \
    apt-get install -y gcc git build-essential

RUN mamba install -c conda-forge conda-pack && \
  mamba env create -f /offline-model/environment.yml

# Use conda-pack to create a  enviornment in /venv:
RUN conda-pack -n offline-model -o /tmp/env.tar && \
...
COPY offline_model/flow.py /opt/prefect/flow.py

RUN chmod +x /usr/local/bin/_entrypoint.sh

RUN source /venv/bin/activate && \
  python -m pip install /offline-model
